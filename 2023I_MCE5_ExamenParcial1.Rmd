...
**Respuesta:**
Suponiendo que voy a replicar el estudio en la ciudad de Tena seria: La facilitacion entre plantas afecta la diversidad y el rendimiento de las especies en una comunidad vegetal
...
sitios = c("Atacapi", "Muyuna", "Alto Tena")
...
**Respuesta:**
Las variables dependientes son la diversidad, el rendimiento de las especies en la comunidad vegetal y afinidad de las especies al sustrato. Las variables independientes incluyen la presencia o ausencia de facilitación entre plantas. 
...
- ¿Cuál es el efecto de las variables ambientales en la i) abundancia de ii) ocurrencia y iii) proporción de la especie *Galumma sp.*?
```{r}
library(ggplot2)
ggplot(data = mites, aes(x = SubstrateDensity, y = AbundanceGalumma, 
                          col = AbundanceGalumma)) +
  geom_point()
```
La figura muestra que a medida que aumenta la densidad del sustrato, la abundancia no aumenta al apar 

```{r}
library(ggplot2)
ggplot(data = mites, aes(x = SubstrateDensity, y = Ocurrence, 
                          col = AbundanceGalumma)) +
  geom_point()
```
La ocurrencia y la densidad del suustrato no se encuentran relacionados 
```{r}
library(ggplot2)
ggplot(data = mites, aes(x = SubstrateDensity, y = Proportion, 
                          col = AbundanceGalumma)) +
  geom_point()
```
Cuando la proporcion es baja se mantiene en su mayoria constante la densidad del sustrato
...
$H_{01}$: La abuandancia de Galumma tiene relacion con la ocurrencia 

```{r verificarH01}
h01 <- lm(AbundanceGalumma ~ Ocurrence, data = mites)
summary(h01)
```
Con un nivel de confianza de casi el 50%  la abundancia tiene relacion con la ocurrencia 
...
```{r}
ggplot(data = mites, aes(x = AbundanceGalumma, y = Ocurrence)) +
  geom_point() + geom_smooth(method = "lm") + theme_classic() + 
  labs(x = "Abundancia de Galumma", y = "Ocurrencia")
```
```{r}

glm1 <- glm(Ocurrence~AbundanceGalumma, family = binomial (link = "logit"), data = mites)
summary(glm1)
```
